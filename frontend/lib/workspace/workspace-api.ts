...
export async function loadWorkspaceLayout(projectId: string): Promise<WorkspaceLayoutDocument | null> {
...
export async function saveWorkspaceLayout(projectId: string, layout: WorkspaceLayoutDocument): Promise<void> {
  const response = await authenticatedFetch(`${API_BASE_URL}/api/v1/projects/${projectId}/workspace`, {
    method: "PUT",
