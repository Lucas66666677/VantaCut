...
export function useRetentionPrediction(timelineId?: string, userId?: string) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/analysis/timelines/${timelineId}/retention-prediction`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, refresh: true }),
      });
