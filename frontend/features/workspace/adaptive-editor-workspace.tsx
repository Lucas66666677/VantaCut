...
import { StudioExportPanel } from "@/features/export/studio-export-panel";
...
export function AdaptiveEditorWorkspace({ timeline, timelineId, projectId, projectStatus }: AdaptiveEditorWorkspaceProps) {
  ...
  // The id of the asset the media bin last finished uploading. Held here
  // rather than inside the bin because the export panel is a sibling, and
  // there is no route that could list it back afterwards.
  const [upload, setUpload] = useState<{ startedAt: number; assetId?: string } | undefined>(undefined);
  ...
  return <main className="min-h-screen bg-[var(--lr-color-background)] text-[var(--lr-color-text-primary)]">
    ...
    <div className="mx-auto max-w-[96rem] px-4 py-4 md:px-6">
      {projectStatus === "error" && <p role="alert" className="mb-4 border border-[var(--lr-color-warning)] bg-[var(--lr-color-surface)] p-3 text-xs text-[var(--lr-color-warning)]">雲端工作區暫時無法建立；你仍可在本機編輯，但這次加入的素材只會保存在此瀏覽器。</p>}
      ...
      {mode === "welcome" ? <div className="mt-4 space-y-3"><LocalMediaBin projectId={projectId} onUploadStarted={() => setUpload({ startedAt: Date.now() })} onAssetUploaded={(id) => setUpload((current) => (current ? { ...current, assetId: id } : { startedAt: Date.now(), assetId: id }))} /><StudioExportPanel projectId={projectId} assetId={upload?.assetId} uploadStartedAt={upload?.startedAt} /><SemanticMediaBin projectId={projectId} /><p className="text-center text-sm text-[var(--lr-color-text-muted)]">{projectId ? "拖入素材後可立即開始；雲端同步會在背景完成。" : "拖入素材後可立即開始；素材只會保存在此瀏覽器。"}</p></div> : <>
