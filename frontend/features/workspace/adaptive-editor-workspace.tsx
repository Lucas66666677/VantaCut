...
export function AdaptiveEditorWorkspace({ timeline, timelineId, projectId, projectStatus }: AdaptiveEditorWorkspaceProps) {
  ...
  return <main className="min-h-screen bg-[var(--lr-color-background)] text-[var(--lr-color-text-primary)]">
    ...
    <div className="mx-auto max-w-[96rem] px-4 py-4 md:px-6">
      ...
      {mode === "welcome" ? <div className="mt-4 space-y-3"><LocalMediaBin projectId={projectId} onAssetUploaded={setUploadedAssetId} /><StudioExportPanel projectId={projectId} assetId={uploadedAssetId} /><SemanticMediaBin projectId={projectId} /><p className="text-center text-sm text-[var(--lr-color-text-muted)]">{projectId ? "拖入素材後可立即開始；雲端同步會在背景完成。" : "拖入素材後可立即開始；素材只會保存在此瀏覽器。"}</p></div> : <>
