...
export function DockableWorkspace({ mode, enabledPanels, timeline, timelineId, projectId, userId }: { mode: WorkspaceMode; enabledPanels: WorkspaceModuleId[]; timeline: TimelineClipInput[]; timelineId?: string; projectId?: string; userId?: string }) {
  ...
  return <section ref={rootRef} className="relative mt-4 min-h-[680px] overflow-hidden rounded-[var(--lr-radius-md)] border border-[var(--lr-color-border)] bg-[var(--lr-color-background)]"><div className="h-[680px] min-h-0 p-2">{visibleTree ? <DockNodeView node={visibleTree} setSlot={setSlot} /> : <div className="grid h-full place-items-center text-sm text-[var(--lr-color-text-muted)]">請從上方啟用至少一個面板。</div>}</div><div ref={floatingLayerRef} className="pointer-events-none absolute inset-0 z-40 [&>[data-dock-panel]]:pointer-events-auto" />{dragGuide && <div className="pointer-events-none absolute inset-2 z-30 rounded-[var(--lr-radius-sm)] border-2 border-[var(--lr-color-primary)] bg-[var(--lr-color-primary-soft)]/30" data-dock-guide={`${dragGuide.targetId}:${dragGuide.edge}`} />}{portals}</section>;
}
