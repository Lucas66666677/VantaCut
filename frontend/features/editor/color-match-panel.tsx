...
export function ColorMatchPanel({ timelineId, projectId, sourceAssetId, previewSource, previewFrameVersion = 0 }: ColorMatchPanelProps) {
  ...
  const create = async () => {
    ...
    try {
      const uploadResponse = await authenticatedFetch(`${API_URL}/api/v1/media/upload-url`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ project_id: projectId, filename: file.name, size_bytes: file.size, content_type: file.type || "image/jpeg", media_type: "image" }) });
      ...
      const confirm = await authenticatedFetch(`${API_URL}/api/v1/media/confirm-upload`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ asset_id: upload.asset_id }) });
      if (!confirm.ok) throw new Error("參考圖確認失敗");
      const matchResponse = await authenticatedFetch(`${API_URL}/api/v1/timelines/${timelineId}/color-match`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reference_image_asset_id: upload.asset_id, source_asset_id: sourceAssetId, intensity: 100, lut_size: 33 }) });
