...
export function SpeedCurvePanel({ clip, timelineId }: SpeedCurvePanelProps) {
  ...
  const persist = async () => {
    if (!timelineId) { setError("請使用已儲存的 Timeline 後再套用變速。"); return; }
    setPending(true); setError(null);
    try { const response = await authenticatedFetch(`${API_URL}/api/v1/timelines/${timelineId}/speed-curves`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ curves: [curve] }) }); if (!response.ok) throw new Error("無法儲存速度曲線"); } catch (cause) { setError(cause instanceof Error ? cause.message : "無法儲存速度曲線"); } finally { setPending(false); }
  };
