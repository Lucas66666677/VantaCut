...
export function OneClickPortraitMattingButton({ mediaAssetId, userId, frameTime, onQueued }: OneClickPortraitMattingButtonProps) {
  ...
  const separate = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/v1/media/${mediaAssetId}/matting`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: userId, mode: "click", frame_time: frameTime, points: [{ x: .5, y: .42, positive: true }], use_proxy: true, feather_pixels: 2.5, despill_strength: .65 }),
      });
