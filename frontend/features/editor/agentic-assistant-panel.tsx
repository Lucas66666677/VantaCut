...
export function AgenticAssistantPanel({
  timelineId, userId, snapshot, selectedClipId, onAccept,
  ...
  const askAgent = async () => {
    if (!timelineId || !userId || !instruction.trim()) return;
    setPending(true); setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/timelines/${timelineId}/agent-preview`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: userId, instruction, timeline_context: compactContext }),
      });
  ...
  return <aside className="rounded-xl border border-fuchsia-300/25 bg-gradient-to-b from-fuchsia-950/25 to-zinc-950 p-4">
    ...
    <button type="button" disabled={!timelineId || !userId || pending || !instruction.trim()} onClick={() => void askAgent()} className="mt-2 w-full rounded-lg bg-fuchsia-200 px-3 py-2 text-xs font-bold text-zinc-950 disabled:opacity-40">{pending ? "正在讀取時間軸…" : "產生安全提案"}</button>
