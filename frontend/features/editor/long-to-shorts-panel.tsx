...
export function LongToShortsPanel({ timelineId, sourceAssetId }: { timelineId: string; userId: string; sourceAssetId?: string }) {
  const [record, setRecord] = useState<Record>({ status: "idle", shorts: [] }); const [pending, setPending] = useState(false);
  const refresh = async () => {
    const response = await authenticatedFetch(`${API_URL}/api/v1/timelines/${timelineId}/long-to-shorts`);
