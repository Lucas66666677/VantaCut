...
export function AIStickerPanel({ timelineId, userId, onChanged }: AIStickerPanelProps) {
  ...
  const recommend = async () => {
    ...
    try { const response = await fetch(`${API_URL}/api/v1/timelines/${timelineId}/recommend-stickers`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, enabled }) }); const data = await response.json() as StickerResponse; if (!response.ok) throw new Error(data.detail ?? "無法推薦貼紙"); applyResponse(data); setMessage(`已建立 ${data.items.length} 個 AI 貼紙建議`); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "無法推薦貼紙"); } finally { setPending(false); }
  };
  const toggle = async (next: boolean) => {
    setPending(true); setMessage(null);
    try { const response = await fetch(`${API_URL}/api/v1/timelines/${timelineId}/ai-stickers/enabled`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId, enabled: next }) }); const data = await response.json() as StickerResponse; if (!response.ok) throw new Error(data.detail ?? "無法更新貼紙開關"); applyResponse(data); } catch (cause) { setMessage(cause instanceof Error ? cause.message : "無法更新貼紙開關"); } finally { setPending(false); }
  };
