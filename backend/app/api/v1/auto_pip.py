...
def request_auto_pip(timeline_id: UUID, payload: AutoPipRequest, db: Session = Depends(get_db)) -> AutoPipResponse:
    timeline = _timeline_for_user(db, timeline_id, payload.user_id)
    task = configure_auto_pip.delay(str(timeline.id), str(payload.main_asset_id), str(payload.selfie_asset_id), payload.model_dump(mode="json", exclude={"user_id", "main_asset_id", "selfie_asset_id"}))
...
def add_auto_pip_overlay(timeline_id: UUID, payload: AutoPipOverlayRequest, db: Session = Depends(get_db)) -> AutoPipOverlayResponse:
    timeline = _timeline_for_user(db, timeline_id, payload.user_id)
    ...
    overlay = {"id": overlay_id, **payload.model_dump(mode="json", exclude={"user_id"}), "animation": {"kind": "vector_draw", "draw_seconds": .28, "fade_out_seconds": .18}}
