...
def request_broll_generation(timeline_id: UUID, payload: BRollGenerationRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> VideoGenerationTaskResponse:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None: raise HTTPException(status_code=404, detail="Timeline not found")
    if timeline.project.owner_id != current_user.id: raise HTTPException(status_code=403, detail="User cannot generate B-Roll for this timeline")
    ...
    task = generate_broll.delay(str(timeline.id), payload.model_dump(mode="json"))
...
def request_video_outpaint(payload: VideoOutpaintRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> VideoGenerationTaskResponse:
    ...
    if asset.project.owner_id != current_user.id: raise HTTPException(status_code=403, detail="User cannot outpaint this media asset")
    ...
    task = outpaint_video.delay(str(asset.id), payload.model_dump(mode="json", exclude={"media_asset_id"}))
