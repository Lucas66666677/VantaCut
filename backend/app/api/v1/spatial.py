...
def _owned(asset: MediaAsset, current_user: User) -> None:
    if asset.project.owner_id != current_user.id: raise HTTPException(status_code=403, detail="User cannot access this spatial scene")
...
def request_spatial_reconstruction(media_asset_id: UUID, payload: SpatialReconstructionRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SpatialTaskResponse:
    asset = db.get(MediaAsset, media_asset_id)
    if asset is None: raise HTTPException(status_code=404, detail="Media asset not found")
    _owned(asset, current_user)
    if asset.status != MediaStatus.READY: raise HTTPException(status_code=409, detail="Media asset is not ready")
    task = reconstruct_spatial_scene.delay(str(asset.id), payload.model_dump(mode="json"))
    return _response(task.id, asset)


@router.get("/{media_asset_id}/spatial-scene")
def get_spatial_scene(media_asset_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    asset = db.get(MediaAsset, media_asset_id)
    if asset is None: raise HTTPException(status_code=404, detail="Media asset not found")
    _owned(asset, current_user)
...
def request_virtual_camera_render(media_asset_id: UUID, payload: VirtualCameraRenderRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SpatialTaskResponse:
    asset = db.get(MediaAsset, media_asset_id)
    if asset is None: raise HTTPException(status_code=404, detail="Media asset not found")
    _owned(asset, current_user)
    ...
    task = render_spatial_virtual_camera.delay(str(asset.id), payload.model_dump(mode="json"))
    return _response(task.id, asset)
