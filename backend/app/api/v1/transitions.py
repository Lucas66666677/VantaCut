...
def _owned(timeline: Timeline, current_user: User) -> None:
...
def update_transitions(timeline_id: UUID, payload: TimelineTransitionsRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None: raise HTTPException(status_code=404, detail="Timeline not found")
    _owned(timeline, current_user)
...
def request_transition_asset(timeline_id: UUID, transition_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> TransitionBuildResponse:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None: raise HTTPException(status_code=404, detail="Timeline not found")
    _owned(timeline, current_user)
