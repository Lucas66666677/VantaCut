...
def request_localized_dub(timeline_id: UUID, payload: LocalizedDubRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> LocalizedDubResponse:
    timeline, profile = db.get(Timeline, timeline_id), db.get(VoiceProfile, payload.voice_profile_id)
    if timeline is None: raise HTTPException(status_code=404, detail="Timeline not found")
    if timeline.project.owner_id != current_user.id: raise HTTPException(status_code=403, detail="User cannot localize this timeline")
