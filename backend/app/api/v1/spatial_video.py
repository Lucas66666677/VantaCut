...
def request_spatial_video_export(
    timeline_id: UUID, payload: SpatialVideoExportRequest, db: Session = Depends(get_db),
    ...
    job = SpatialVideoJob(
        ...
        options_json=payload.model_dump(mode="json", exclude={"user_id", "source_render_job_id"}),
    )
