...
def request_travel_map(timeline_id: UUID, payload: TravelMapRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> TravelMapTaskResponse:
    ...
    task = generate_travel_map.delay(str(timeline.id), payload.model_dump(mode="json"))
...
def travel_map_status(timeline_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> TravelMapStatusResponse:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None: raise HTTPException(status_code=404, detail="Timeline not found")
