...
from app.models.entities import MediaAsset, MediaEmbeddingSegment, MediaStatus, MediaType, Project, User
...
def get_derived_preview(media_asset_id: str, job_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> DerivedPreviewResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=422, detail="Invalid media asset id") from exc
...
def search_media_semantically(
    ...
    if project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot search this project's media")
...
def semantic_media_grid(payload: MediaSemanticGridRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> MediaSemanticGridResponse:
    ...
    if project is None:
        raise HTTPException(status_code=404, detail="Project not found")
    if project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot view this project's media grid")
    assets = db.scalars(
        select(MediaAsset)
...
def _create_uploading_asset(payload: UploadURLRequest, current_user: User, db: Session) -> MediaAsset:
    project = db.get(Project, payload.project_id)
    if project is None:
        raise HTTPException(status_code=404, detail="Project not found")
    if project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot upload media to this project")
...
def get_multipart_part_url(payload: MultipartPartURLRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> MultipartPartURLResponse:
    ...
    if asset is None:
        raise HTTPException(status_code=404, detail="Media asset not found")
    if asset.project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot upload parts for this media asset")
...
def complete_multipart_media_upload(payload: MultipartCompleteRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> MediaAssetResponse:
    asset = db.get(MediaAsset, payload.asset_id)
    if asset is None:
        raise HTTPException(status_code=404, detail="Media asset not found")
    if asset.project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot complete this media upload")
    if asset.status != MediaStatus.UPLOADING:
...
def confirm_upload(
    payload: ConfirmUploadRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
) -> MediaAssetResponse:
    asset = db.get(MediaAsset, payload.asset_id)
    if asset is None:
        raise HTTPException(status_code=404, detail="Media asset not found")
    if asset.project.owner_id != current_user.id:
        raise HTTPException(status_code=403, detail="User cannot confirm this media upload")
    if asset.status != MediaStatus.UPLOADING:
