...
def request_audio_sync(timeline_id: UUID, payload: AudioSyncRequest, db: Session = Depends(get_db)) -> AudioSyncTaskResponse:
    ...
    task = align_external_audio.delay(str(timeline.id), payload.model_dump(mode="json", exclude={"user_id"})); base = f"/api/v1/projects/{timeline.project_id}/status"
...
def audio_sync_status(timeline_id: UUID, user_id: UUID, db: Session = Depends(get_db)) -> AudioSyncStatusResponse:
