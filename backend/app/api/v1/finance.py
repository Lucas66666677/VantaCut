...
def _authorised_timeline(timeline_id: UUID, user_id: UUID, db: Session) -> Timeline:
...
def create_finance_track(timeline_id: UUID, payload: FinanceTrackRequest, db: Session = Depends(get_db)) -> FinanceTrackResponse:
    timeline = _authorised_timeline(timeline_id, payload.user_id, db)
    finance_track_id = str(uuid.uuid4())
    track = {"id": finance_track_id, "status": "processing", **payload.model_dump(mode="json", exclude={"user_id"})}
...
def list_finance_tracks(timeline_id: UUID, user_id: UUID, db: Session = Depends(get_db)) -> dict[str, object]:
    timeline = _authorised_timeline(timeline_id, user_id, db)
