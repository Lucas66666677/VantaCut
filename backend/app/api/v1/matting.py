...
def request_video_matting(
    media_asset_id: UUID, payload: VideoMattingRequest, db: Session = Depends(get_db),
    ...
    task = generate_video_matte.delay(str(asset.id), payload.model_dump(mode="json", exclude={"user_id"}))
