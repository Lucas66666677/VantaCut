...
def analyze(media_asset_id: UUID, payload: SpatialTrackingRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SpatialTrackingResponse:
    asset = db.get(MediaAsset, media_asset_id)
    if asset is None or asset.project.owner_id != current_user.id: raise HTTPException(status_code=404, detail="Media asset not found")
...
def add_text(timeline_id: UUID, payload: SpatialTextRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict[str, str]:
    timeline, asset = db.get(Timeline, timeline_id), db.get(MediaAsset, payload.source_asset_id)
    if timeline is None or asset is None or timeline.project.owner_id != current_user.id or asset.project_id != timeline.project_id: raise HTTPException(status_code=404, detail="Timeline or source asset not found")
    ...
    item = {"id": f"spatial-text-{uuid4()}", **payload.model_dump(mode="json"), "depth_key": tracking["depth_key"], "camera_poses_key": tracking["camera_poses_key"], "occlusion_depth": round(1 - payload.z, 4), "animation": {"kind": "camera_locked", "z": payload.z}}
