...
def request_generated_music(timeline_id: UUID, payload: GenerateMusicRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> GenerateMusicResponse:
...
def generated_music_status(timeline_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> GenerateMusicStatusResponse:
    timeline = _timeline_for_user(db, timeline_id, current_user)
