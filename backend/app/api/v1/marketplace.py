...
from app.schemas.marketplace import (
    ApplyLicenseRequest, ApplyLicenseResponse, CheckoutResponse, ConnectOnboardingRequest,
...
def publish_template(payload: PublishMarketplaceTemplateRequest, db: Session = Depends(get_db)) -> MarketplaceTemplateResponse:
    template = db.get(Template, payload.template_id)
    if template is None or template.project.owner_id != payload.creator_id:
    ...
    listing = MarketplaceTemplate(
        template_id=template.id, creator_id=payload.creator_id, slug=payload.slug, title=payload.title,
...
def start_connect_onboarding(payload: ConnectOnboardingRequest, db: Session = Depends(get_db)) -> ConnectOnboardingResponse:
    creator = db.get(User, payload.creator_id)
    if creator is None:
        raise HTTPException(status_code=404, detail="Creator not found")
    existing = db.scalar(select(CreatorConnectAccount).where(CreatorConnectAccount.creator_id == creator.id))
    if existing:
        raise HTTPException(status_code=409, detail="Creator already has a Stripe Connect account")
    try:
        result = create_connect_onboarding(creator_email=creator.email, idempotency_key=f"creator:{creator.id}:connect")
    except MarketplacePaymentError as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    account = result["account"]
    db.add(CreatorConnectAccount(
        creator_id=creator.id, stripe_account_id=result["account_id"],
...
def create_checkout(slug: str, payload: CreateLicenseRequest, db: Session = Depends(get_db)) -> CheckoutResponse:
    ...
    if project is None or project.owner_id != payload.buyer_id:
        raise HTTPException(status_code=403, detail="Buyer cannot apply a template to this project")
    if listing.creator_id == payload.buyer_id:
    ...
    license_row = TemplateLicense(
        marketplace_template_id=listing.id, buyer_id=payload.buyer_id, project_id=project.id,
...
def apply_template_license(license_id: UUID, payload: ApplyLicenseRequest, db: Session = Depends(get_db)) -> ApplyLicenseResponse:
    ...
    if license_row is None or license_row.buyer_id != payload.buyer_id:
