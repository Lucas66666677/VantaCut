...
def create_voice_profile(project_id: UUID, payload: CreateVoiceProfileRequest, db: Session = Depends(get_db)) -> VoiceProfileResponse:
    _owned_project(db, project_id, payload.user_id)
    ...
    profile = VoiceProfile(project_id=project_id, created_by_id=payload.user_id, source_media_asset_id=asset.id, name=payload.name, provider_name=get_voice_clone_provider().name, status=VoiceProfileStatus.QUEUED, language=payload.language, metadata_json={"consent_confirmed": True})
...
def list_voice_profiles(project_id: UUID, user_id: UUID, db: Session = Depends(get_db)) -> list[VoiceProfileResponse]:
...
def request_voice_replacement(timeline_id: UUID, payload: GenerateVoiceReplacementRequest, db: Session = Depends(get_db)) -> VoiceReplacementResponse:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None:
        raise HTTPException(status_code=404, detail="Timeline not found")
    _owned_project(db, timeline.project_id, payload.user_id)
...
def request_voice_morph(timeline_id: UUID, payload: VoiceMorphRequest, db: Session = Depends(get_db)) -> VoiceMorphResponse:
