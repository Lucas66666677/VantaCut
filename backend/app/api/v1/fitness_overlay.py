...
def request_fitness_overlay(timeline_id: UUID, payload: FitnessOverlayRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> FitnessOverlayTaskResponse:
    timeline, source = db.get(Timeline, timeline_id), db.get(MediaAsset, payload.source_asset_id)
    ...
    task = analyze_fitness_reps.delay(str(timeline.id), payload.model_dump(mode="json")); base = f"/api/v1/projects/{timeline.project_id}/status"
...
def fitness_overlay_status(timeline_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> FitnessOverlayStatusResponse:
