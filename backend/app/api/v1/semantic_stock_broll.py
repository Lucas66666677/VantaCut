...
def request_semantic_stock_broll(timeline_id: UUID, payload: SemanticStockBRollRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SemanticStockBRollTaskResponse:
    ...
    db.commit()
    task = generate_semantic_stock_broll.delay(str(timeline.id), payload.model_dump(mode="json"))
...
def semantic_stock_broll_status(timeline_id: UUID, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> SemanticStockBRollStatusResponse:
    timeline = db.get(Timeline, timeline_id)
    if timeline is None:
        raise HTTPException(status_code=404, detail="Timeline not found")
    if timeline.project.owner_id != current_user.id:
