...
def _patch_boundaries(monkeypatch, calls):
    ...
    # configured deployment (both the internal and the browser-facing endpoint)
    # so the upload routes reach their identity checks. The gate's own
    # fail-closed behaviour is covered separately below.
    monkeypatch.setattr(media, "storage_is_configured", lambda: True)
    monkeypatch.setattr(media, "public_storage_is_configured", lambda: True)
...
def test_m11_upload_urls_are_refused_when_only_the_public_endpoint_is_unconfigured(db_session, monkeypatch):
    """The browser-facing endpoint alone is enough to break the flow.

    Every presigned URL is signed against s3_public_endpoint_url. With the
    internal endpoint configured (bucket probe would pass) but the public
    endpoint left at its dev default, the routes still fail closed rather than
    handing the browser a URL against a host it cannot reach.
    """
    calls = []; _patch_boundaries(monkeypatch, calls)
    monkeypatch.setattr(media, "storage_is_configured", lambda: True)
    monkeypatch.setattr(media, "public_storage_is_configured", lambda: False)
    owner = _user(db_session); graph = _graph(db_session, owner); client = _client(db_session)
    project, _derived, part, _complete, _confirm = graph
    upload = {"project_id": str(project.id), "filename": "new.mp4", "size_bytes": 10, "content_type": "video/mp4", "media_type": "video"}
    asset_count = db_session.query(MediaAsset).count()
